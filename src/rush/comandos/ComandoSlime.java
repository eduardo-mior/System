	...
	public boolean onCommand(CommandSender sender, Command cmd, String lb, String[] args) {
	      if (cmd.getName().equalsIgnoreCase("slime")) {
		       ...
		       if (random.nextInt(10) == 0) {
		    	   p.sendMessage(ConfigManager.getConfig("mensagens").getString("SlimeChunk.Esta").replaceAll("&", "§"));
		    	   p.playSound(p.getLocation(), Sound.SLIME_WALK2, 1, 1);
		       } 
		       
		       else {
		    	   p.sendMessage(ConfigManager.getConfig("mensagens").getString("SlimeChunk.NaoEsta").replaceAll("&", "§"));
